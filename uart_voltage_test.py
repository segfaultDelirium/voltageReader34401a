...
def voltageToBytesArray(voltage):
    ...
    data = voltageInDAC_resulution.to_bytes(2, 'big')
    print(f'data= ', data)
    return bytes([0x33, 0x35, 0x80 + len(data)+1, 0x00, *data,  0x80, 0xc0 + len(data)+1])
...
# Send bytes
helloString = "hello"
bytesArray = bytes([0x33, 0x35, 0x80 + len(helloString), *bytes(helloString, 'ascii'), 0x80, 0xc0 + len(helloString) ])
